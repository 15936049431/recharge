...
class TreasureController extends Controller {
	...
	public $location_url = "http://api.reapal.com/mobile/portal"; 
	public $merchant_id = "100000000218171" ;
	//public $merchant_id = "100000000011015" ;
	public $seller_email = "540473825@qq.com" ;
	...
	public $currency = "156" ;
	public $use_key = "0714g5028gg3a20dfaab3ae99367d071b3aaa2g5d2fb8b58ddgefgd41053a884" ;
	//public $use_key = "e977ade964836408243b5g2444848f7b39d09fb41c77ae2e327ffb16f905e117" ;
	public $public_key = "/paykey/treasure_rb/publickey.pem" ;
	public $private_key = "/paykey/treasure_rb/privatekey.pem" ;
	public $rb_public_key = "/paykey/treasure_rb/itrus001.pem" ;
	public $rb_private_key = "/paykey/treasure_rb/itrus001_pri.pem" ;
	public $ending_key = "" ;
	...
	public function recharge(Request $request){
		...
		$this->ending_key = $this->generateAESKey();
	...
	public function check_and_insert($data){
		...
		if(!empty($user_result) && empty($is_have) && strpos(Request::server("HTTP_REFERER"),"www.jinzhangguinb.com") > 0 ){
			...
			$log_add['status'] = "0" ;
			$log_add['payment'] = "67";
			$log_add['money'] = $data['total_fee']/100;
			$log_add['fee'] = "0";
	...
	public function recharge_notify(Request $request){
		$return_keys = $this->rebuild_rb_key($request::get("encryptkey"));
		$return_result = $this->rebuild_rb_data($request::get("data"),$return_keys);
		$return_result_array = json_decode($return_result,true);
		if(!empty($return_result_array)){
			$flag = $this->return_and_addbill($return_result_array['order_no'],$return_result);
			$message = "" ;
			if($flag === true){
				$message = "SUCCESS" ;
			}else{
	...
	public function recharge_return(Request $request){
		...
		if(!empty($return_result_array)){
			...
			$script = "<script>alert('{$message}');window.location.href='http://www.jinzhangguinb.com/front_account/toMyAccount.dql?device=weixin'</script>";
			echo $script;exit;
		}
		return false;
	}
	...
	public function rebuild_rb_data($treasure_data,$ending_key){
		$decrypted= mcrypt_decrypt(MCRYPT_RIJNDAEL_128,$ending_key,base64_decode($treasure_data),MCRYPT_MODE_ECB);
		$dec_s = strlen($decrypted);
		$padding = ord($decrypted[$dec_s-1]);
		$treasure_data = substr($decrypted, 0, -$padding);
	...
	public function rebuild_rb_key($treasure_key){
		...
		$res = openssl_private_decrypt(base64_decode($treasure_key),$return_keys,$private_key_yes);
		return $return_keys;
	}
	
	public function build_rb_key($treasure_key){
		$public_key = file_get_contents(resource_path().$this->rb_public_key);
